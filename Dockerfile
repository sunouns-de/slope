...
RUN addgroup -g 1234 sunouns
RUN adduser -G sunouns -u 1234 --disabled-password sunouns

COPY ./public-html/ /usr/local/apache2/htdocs/

RUN sed -i '/Listen 80/c\Listen 8000' $HTTPD_PREFIX/conf/httpd.conf
#RUN echo Listen 8001 >> $HTTPD_PREFIX/conf/httpd.conf
RUN cat $HTTPD_PREFIX/conf/httpd.conf

RUN chown -R sunouns:sunouns /usr/local/apache2